...
public:
    ...
    void viewAvailableElections()
    {
        ...
        {
            if (!getline(electionFile, name))
                break;
            if (!getline(electionFile, type))
                break;
            if (!getline(electionFile, status))
                break;
        ...
        if (candidateFile.is_open())
        {
            ...
            while (getline(candidateFile, candElectionId) && candidateCount < MAX_CANDIDATES)
            {
                if (!getline(candidateFile, candidateId))
                    break;
                if (!getline(candidateFile, candidateName))
                    break;
                if (!getline(candidateFile, candidateParty))
                    break;
...
public:
    Administrator(const string &username, const string &password, const string &userId,const string &name = "", const string &role = "")
        : User(username, password, userId)
    {
        adminName = name;
        adminRole = role;
    ...
    void viewElectionResults()
    {
        ...
        if (voteFile.is_open())
        {
            ...
            {
                if (!getline(voteFile, voteCandidateId))
                    break;
                if (!getline(voteFile, voterId))
                    break;
    ...
    void listAllVoters()
    {
        ...
        while (getline(voterFile, username))
        {
            if (!getline(voterFile, password))
                break;
            if (!getline(voterFile, CNIC))
                break;
            if (!getline(voterFile, name))
                break;
            if (!getline(voterFile, address))
                break;
